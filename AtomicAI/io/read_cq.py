...
def read_cq(file_name):
    ...
    sorted_coords = sorted(coords, key=lambda x: x[3], reverse=False)

    sorted_species = {}
    for cq_sy_no, symbol in species_type.items():
        sorted_species.update({specie:specie_count for specie, specie_count in species.items() if specie==symbol})

    data['species'] = dict(sorted_species) 

    atom_i = 0
    positions = []
    for specie, specie_count in sorted_species.items():
        xyz_data = []
        for i in range(0, specie_count):
            item = sorted_coords[atom_i]
